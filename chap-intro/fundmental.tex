...
实际计算中会进一步定义反对称的矩阵元
\begin{equation}
  V^\text{AS}_{\alpha\beta\gamma\delta}=\langle\alpha\beta|\hat{V}|\gamma\delta\rangle=\frac12[(\alpha\beta|-(\beta\alpha|]\hat{V}[|\gamma\delta)-|\delta\gamma)]=V_{\alpha\beta\gamma\delta}-V_{\alpha\beta\delta\gamma},
\end{equation}
反对称矩阵元的性质为
\begin{equation}
...
\section{角动量耦合}
前面提到的直积态是一般的讨论，而在原子核的实际计算中，角动量及其耦合是非常重要的概念。描述单粒子态的量子数包括 $n,l,j,m,t_z$，单粒子态最详细的写法为 $|nljt_zm\rangle$，但在研究角动量耦合时，$n,l,t_z$ 这些量子数并不重要，用 $\zeta$ 代表这些量子数，可以把单粒子态写为 $|\zeta_ij_im_i\rangle$。还有一种写法是用 $k$ 代表 $\zeta$ 与 $j$，将单粒子态写为 $|km\rangle$，$k$ 相当于代码中 $j$-scheme 单粒子轨道的唯一编号，这种写法更加简洁，不过需要注意在角动量耦合时 $k_\alpha$ 对应的角动量就是 $j_\alpha$。在后续推导中使用的基本就是这种记法。
...
\begin{align}
  \langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle
  &=\frac1{\sqrt{2j_f+1}}C_{j_im_ikq}^{j_fm_f}\langle\zeta_fj_f||T^k||\zeta_ij_i\rangle\label{eqs:w-e-cg1}\\
  &=\frac{(-1)^{k-j_i+j_f}}{\sqrt{2j_f+1}}C_{kqj_im_i}^{j_fm_f}\langle\zeta_fj_f||T^k||\zeta_i j_i\rangle\label{eqs:w-e-cg2}\\
  &=(-1)^{j_f-m_f}
  \begin{pmatrix}
    j_f&k&j_i\\-m_f&q&m_i
  \end{pmatrix}\langle\zeta_fj_f||T^k||\zeta_i j_i\rangle,\label{eqs:w-e-3j}
\end{align}
\end{subequations}
...
通过 \ref{eqs:wigner-eckart} 能够很方便地从角动量耦合的角度理解为什么标量算符 $H$ 不会改变角动量，又因为 CG 系数隐含的要求 $m_i+q=m_f$，$H$ 也不会改变 $m$ 量子数。因此哈密顿量的两体部分的矩阵元是
\begin{equation}
...
\begin{align}
  \langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle
  =(-1)^{j_i-m_i}\hat{k}^{-1}C_{j_fm_fj_i-m_i}^{kq}\langle\zeta_fj_f||T^k||\zeta_ij_i\rangle,
\end{align}
由此在左右两侧同乘 $C_{j_fm_fj_i-m_i}^{kq}$ 并对 $m_i,m_f$ 求和，得到
\begin{subequations}\label{eqs:wigner-eckart-reverse}
\begin{align}
  \langle\zeta_fj_f||T^k||\zeta_ij_i\rangle&=\hat{k}\sum_{m_im_f}(-1)^{j_i-m_i}C_{j_fm_fj_i-m_i}^{kq}\langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle\label{eqs:w-e-reverse-cg}\\
  &=\hat{k}^2\sum_{m_im_f}(-1)^{j_f-m_f}
  \begin{pmatrix}
    j_f&k&j_i\\-m_f&q&m_i
  \end{pmatrix}\langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle\label{eqs:w-e-reverse-3j1}\\
  &=\sum_{m_im_fq}(-1)^{j_f-m_f}
  \begin{pmatrix}
    j_f&k&j_i\\-m_f&q&m_i
  \end{pmatrix}\langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle,\label{eqs:w-e-reverse-3j2}
\end{align}
\end{subequations}
在 CG 系数转换为 $3j$ 符号时，用到了 $q=m_f-m_i$ 且 $q$ 为整数的条件。得到 \ref{eqs:w-e-reverse-3j2} 用到的 $3j$ 符号正交关系为 \ref{app:3j-ortho-summ1m2m3}。
...
\begin{equation}
  \langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle=(-1)^{k-q}(-1)^{j_f-m_f}C_{j_im_ij_f-m_f}^{k-q}\langle\zeta_fj_f||T^k||\zeta_ij_i\rangle_\mathrm{Kuo},
\end{equation}
前面的相位可以吸收进 CG 系数中，使定义更加清晰，
\begin{equation}
    \langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle=(-1)^{j_i+m_i}C_{j_i-m_ij_fm_f}^{kq}\langle\zeta_fj_f||T^k||\zeta_ij_i\rangle_\mathrm{Kuo},\label{eqs:w-e-kuo}
\end{equation}
转为 $3j$ 符号，
\begin{equation}
    \langle\zeta_fj_fm_f|T^k_q|\zeta_ij_im_i\rangle=(-1)^{j_i-m_f+k}\hat{k}
    \begin{pmatrix}
      j_f&k&j_i\\-m_f&q&m_i
    \end{pmatrix}
    \langle\zeta_fj_f||T^k||\zeta_ij_i\rangle_\mathrm{Kuo},\label{eqs:w-e-kuo-3j}
\end{equation}
与 Edmonds 约定 \ref{eqs:w-e-3j} 的 $3j$ 符号一致，可以很简单地约去，得到Kuo 的约定与 Edmonds 满足的关系为
\begin{equation}
  \langle\zeta_fj_f||T^k||\zeta_ij_i\rangle_\mathrm{Edmonds}=(-1)^{j_i-j_f+k}\hat{k}\langle\zeta_fj_f||T^k||\zeta_ij_i\rangle_\mathrm{Kuo},
\end{equation}
