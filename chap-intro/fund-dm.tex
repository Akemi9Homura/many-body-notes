...
另外，当左右两侧的态不同时，也可以定义跃迁密度矩阵。虽然核物理中确实是这么算的，但是不知道在什么教材或文章中有明确的定义与公式。因此在这里采用量子化学文章 \cite{2017blunt} 的记号，他们的定义与密度矩阵的基本定义非常自洽。跃迁的初态与末态分别记为 \(\ket{\Psi_i}\) 与 \(\ket{\Psi_f}\)，最标准，与 \cite{brown,2007suhonen} 相一致的记号就是把末态 \(\ket{\Psi_f}\) 放在左侧，初态 \(\ket{\Psi_i}\) 放在右侧。
